  ...
  return (
    <Card className="border-border/40 bg-card/60 backdrop-blur-md shadow-2xl shadow-black/5 rounded-[2.5rem] h-full flex flex-col hover:bg-card/80 transition-all duration-300">
      ...
      <CardContent className="flex-1 flex flex-col justify-between p-8 pt-0">
        <div className="space-y-8 flex-1 max-h-[250px] overflow-y-auto">
          {activeGoals.length > 0 ? (
            activeGoals.map((goal: any, i: number) => {
              ...
              return (
                <div key={goal.id || i} className="space-y-3">
                  <div className="flex justify-between items-center text-sm">
                    <div className="font-black text-foreground uppercase tracking-widest text-[10px] truncate max-w-[150px]">{goal.name || goal.title}</div>
                    <div className="flex items-center gap-1 shrink-0">
                      <span className="font-black text-foreground text-xs">
                        {maskValue(current, formatter.format)}
                      </span>
                      <span className="text-muted-foreground/60 font-black text-[10px] uppercase tracking-widest">/ {maskValue(target, formatter.format)}</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-4">
                    ...
                    <span className="text-[10px] font-black text-muted-foreground/60 uppercase tracking-widest w-8 text-right shrink-0">
                      {ratio.toFixed(0)}%
                    </span>
                  </div>
                </div>
            ...
            <p className="text-xs font-black uppercase tracking-widest text-muted-foreground/60 text-center py-4">
              Sem objetivos definidos.
            </p>
          )}
        </div>
        <div 
          className="mt-8 pt-6 border-t border-border/40 flex items-center justify-center text-[10px] font-black uppercase tracking-widest text-foreground cursor-pointer hover:text-primary transition-colors"
